...
Names such as Codagent, Eval, Bug, Owner, Ready, `eval-request`, and `factory-hold` describe the Codagent deployment. Organization and repository identities, Project destinations, request markers, native issue types, bypass markers, and field/option mappings SHALL be configurable as specified in `factory-operations`. The eval routing requirements below move here from `factory-eval-intake` without behavior change.
