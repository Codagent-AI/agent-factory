...
These requirements describe the eval handler and its initial `and-scene` suite integration. Runner/Skills inputs and suite paths SHALL remain specific to that integration rather than become assumptions of the generic factory lifecycle. Repository locations and local paths SHALL be configurable.
