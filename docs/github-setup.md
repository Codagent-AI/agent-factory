...
The configured `agent-evals` harness SHA is a real provisional execution pin, not a local `HEAD` or branch. Routing does not establish suite readiness. Before execution, update it to the integrated immutable revision containing the score-failure contract and calibration-gate removal; verify the selected revision against the suite before unpausing admission.
